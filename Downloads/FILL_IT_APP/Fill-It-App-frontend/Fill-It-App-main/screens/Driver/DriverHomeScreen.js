...
export default function DriverHomeScreen({ navigation }) {
  ...
  const { tripRequests, addTripRequest } = useTripRequests();
      ...
      try {
        ...
        if (granted === PermissionsAndroid.RESULTS.GRANTED) {
          Geolocation.getCurrentPosition(
            ...
            error => {
              console.log(error);
              setRouteError('Could not get current location. Enable location services.');
            },
  ...
  const fetchRoute = async () => {
    ...
    try {
      ...
      if (data.routes && data.routes.length > 0) {
        ...
        setRouteError(data.status ? `No route found: ${data.status}` : 'No route found for the given locations.');
      }
    } catch (error) {
      console.error(error);
      setRouteError('Failed to fetch route. Check your internet or API key.');
    } finally {
  ...
  const handleSearch = async () => {
    ...
    const existing = tripRequests.find(trip => trip.from === from);
    if (existing) {
      setMatchedRequest(existing);
    ...
    <SafeAreaView style={styles.container}>
      <View style={StyleSheet.absoluteFillObject}>
        {region ? (
          ...
          >
            {!routeCoordinates.length && <Marker coordinate={region} title="You are here" />}
      ...
      <View style={styles.card}>
        {}
        {showTripRequest && matchedRequest && (
          <View style={[styles.tripRequestBlock, { backgroundColor: '#000', padding: 16, borderRadius: 10, marginBottom: 16 }]}> 
            <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 16, marginBottom: 8 }}>Trip Request Found</Text>
            <Text style={{ color: '#fff' }}>From: {matchedRequest.from}</Text>
            <Text style={{ color: '#fff' }}>To: {matchedRequest.to}</Text>
            <Text style={{ color: '#fff' }}>Date: {matchedRequest.date}</Text>
            <Text style={{ color: '#fff' }}>Customer: {matchedRequest.customer}</Text>
            <Text style={{ color: '#fff' }}>Phone: {matchedRequest.phone}</Text>
            <Text style={{ color: '#fff' }}>Status: {matchedRequest.status}</Text>
          </View>
        )}
